#!/bin/bash
# build-check-phase04.sh — Kiem tra COMPILE + chay UNIT TEST cho Phase 4 (Physics Tuning).
# Dung: ./build-check-phase04.sh
#
...
# - Compile sach -> in ket qua [PASS]/[FAIL] (gom nhom 6: Durability) + tong ket.

UNITY="/Applications/Unity/Hub/Editor/6000.4.7f1/Unity.app/Contents/MacOS/Unity"
PROJECT="$(cd "$(dirname "$0")" && pwd)"
LOG="/tmp/cho-noi-build-p4-$(date +%s).log"

echo ""
echo "=================================================="
echo "  CHO NOI MIEN TAY — Build & Test Check (Phase 4)"
echo "=================================================="
...
if [ ! -x "$UNITY" ]; then
  ...
  echo "        Sua bien UNITY trong build-check-phase04.sh cho dung phien ban."
  exit 1
fi
