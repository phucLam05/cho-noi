#!/bin/bash
# build-check.sh — Kiem tra COMPILE + chay UNIT TEST cua project qua Unity batch mode.
# Dung: ./build-check.sh
#
# - Mo project trong batchmode, goi ChoNoi.Editor.BoatTestRunner.Run (se compile toan bo script).
# - Neu co loi compile (error CS....) -> in ra va exit 1.
# - Neu compile sach -> in ket qua [PASS]/[FAIL] cua unit test + tong ket.

UNITY="/Applications/Unity/Hub/Editor/6000.4.7f1/Unity.app/Contents/MacOS/Unity"
PROJECT="$(cd "$(dirname "$0")" && pwd)"
LOG="/tmp/cho-noi-build-$(date +%s).log"

echo ""
echo "=================================================="
echo "  CHO NOI MIEN TAY — Build & Test Check (Phase 2)"
echo "=================================================="
...
if [ ! -x "$UNITY" ]; then
  ...
  echo "        Sua bien UNITY trong build-check.sh cho dung phien ban."
  exit 1
fi
